...
def importar_arquivo(cursor, caminho_arquivo): # Importa um único arquivo .xlsx para o banco SQLite.
    ...
    for _, row in df.iterrows():
        ...
        medida_embalagem = str(row.get("Quantidade")).strip().upper() if pd.notna(row.get("Quantidade")) else None
        ...
        cursor.execute("""
            INSERT INTO produto
            (codigo_barras, nome_produto, medida_embalagem, unidade, valor_unitario)
            VALUES (?, ?, ?, ?, ?)
        """, (codigo, nome_produto, medida_embalagem, unidade, valor_unitario))
